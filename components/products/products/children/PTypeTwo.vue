    ...
    <section class="mb-4 lg:mb-16 relative z-[1] container">
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-4 sm:grid-cols-2">
            <div v-for="(product, index) in productList" :key="index"
                ...
                <NuxtLink :href="'/l/' + landSlug + '/p/' + product.slug" class="h-32 mb-0.5">
                    <img :src="product.image" :alt="product.name" class="object-contain h-full" />
                </NuxtLink>
                <NuxtLink :href="'/l/' + landSlug + '/p/' + product.slug" class="mb-0.5 font-medium lg:mb-1 text-lg sm:line-clamp-1 text-stone-700"> {{ product.name }}
                </NuxtLink>
