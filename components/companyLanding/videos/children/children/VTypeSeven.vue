<template>
    <ul :class="'list-none lg:w-full mb-8 lg:mb-16 ' + (videoList.length == 1 ? 'justify-center flex' : (videoList.length == 2 ? 'flex gap-8 sm:flex-row sm:items-center flex-col lg:grid lg:grid-cols-2 overflow-auto lg:overflow-visible pb-4' : (videoList.length == 3 ? 'flex gap-8 sm:flex-row sm:items-center flex-col lg:grid lg:grid-cols-3 lg:grid-row-2 overflow-auto lg:overflow-visible pb-4 lg:gap-x-4 lg:gap-y-2' : 'flex gap-8 sm:flex-row sm:items-center flex-col lg:grid lg:grid-cols-3 overflow-auto lg:overflow-visible pb-4 lg:gap-4')))">
        ...
        <li v-if="videoList.length > 3" class="flex-none w-full sm:w-[448px] md:w-[480px] lg:w-full">
            <div class="aspect-video size-full flex_center flex-col gap-6 rounded-custom overflow-hidden border border-stone-700 hover:scale-[1.025] duration-300 relative z-[1]">
                <p class="text-base sm:text-lg font-medium text-slate-700 text-center"> برای ورود به آرشیو ویدیو ها کلیک کنید </p>
                <NuxtLink :to="`/l/${slug}/v`" class="h-11 w-44 flex_center bg-normal rounded-custom text-lg font-medium text-white hover:bg-focus"> بیشتر </NuxtLink>
            </div>
        </li>
    </ul>
</template>
