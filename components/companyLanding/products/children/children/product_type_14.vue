<template>
    <section class="w-full grid grid-cols-1 gap-6 lg:grid-cols-4">
        <div v-for="(product, index) in productList.slice(0, 4)" :key="index"
            ...
            <NuxtLink :to="'/l/' + slug + '/p/' + product.slug"
                      class="mb-6 font-normal text-sm line-clamp-2 text-stone-700 text-justify leading-6 lg:h-12">
                {{ product.description }}
            </NuxtLink>
            <div class="grid grid-cols-2 gap-2 w-56 lg:w-full">
                <NuxtLink :to="'/l/' + slug + '/p/' + product.slug"
                          class="sameCategoryBtnStyle castegoryBtnfilled rounded-custom col-span-2"> خرید اقساطی
                </NuxtLink>
                <NuxtLink :to="'/l/' + slug + '/p/' + product.slug"
                          class="sameCategoryBtnStyle categoryBtnEmpty rounded-custom"> مشخصات
                </NuxtLink>
                <NuxtLink :to="'/l/' + slug + '/p/' + product.slug"
                          class="sameCategoryBtnStyle categoryBtnEmpty rounded-custom"> کاتالوگ
                </NuxtLink>
            </div>
        </div>
    </section>
