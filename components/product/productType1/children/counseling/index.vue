<template>
    ...
    <section v-if="showModal"
             class="fixed overflow-auto top-0 right-0 w-full h-full sm:h-auto sm:max-w-[36rem] rounded-custom md:max-w-[40rem] lg:max-w-4xl xl:max-w-5xl sm:top-1/2 sm:right-1/2 sm:translate-x-1/2 sm:-translate-y-1/2 z-[6] sm:overflow-hidden">
        <!-- calculator -->
...
<!--                </svg>-->
                <i class="font-Icomoon text-xl text-stone-700 icon-x-mark"></i>
            </div>
                    ...
                    <section class="mb-8 lg:mb-12">
                        ...
                        <div class="h-2 w-full bg-stone-200 rounded-full mb-4 relative">
                            <input v-bind:style="{ background: priceSliderBackground }" type="range"
                                   ...
                                   :min="loanMin" v-bind:max="loanMax"
...
export default {
    ...
    props: {
        ...
        closeModal: Function,
        changeCounselingStep: Function,
        counselingStep: [String, Number]
    },
    components: {FormIconOne, FormIconTwo},
    setup(){
        ...
        const submitForm = () => {
            ...
            if (!phoneAlert.value && !nameAlert.value && !categoryAlert.value && !facilityAlert.value) {
                ...
                console.log(body);
                $fetch(`https://paye1.com/api/l/facilities-request`, {
                    method: "POST",
                    body: body,
                }).then(response => {
