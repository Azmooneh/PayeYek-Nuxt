...
<script>
import { usePayeYek } from '~/store/index.js';

export default {
    name: 'Paye 1 Header',
    setup(){
        const payeYekStore = usePayeYek();
        const openSidebar = ref(computed(() => payeYekStore.openSidebar));
        const toggleDarkmode = () => {
            payeYekStore.toggleDarkmode();
        }
        const toggleSidebar = () => {
            payeYekStore.toggleSidebar();
        }

        const isDark = ref(computed(() => payeYekStore.darkMode));
